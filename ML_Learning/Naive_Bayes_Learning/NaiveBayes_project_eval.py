...
import jieba                  #分词包                  
from sklearn.feature_extraction.text import CountVectorizer     #词频统计表，把 评论内容 转化成 词频矩阵
from sklearn.naive_bayes import MultinomialNB                   #朴素贝叶斯对象
from sklearn.metrics import accuracy_score              #模型评估

#1. 读取文件，获取原始数据
df = pd.read_csv('E:\\To_learn_Ai\\ML_Learning\\Naive_Bayes_Learning\\data\\书籍评价.csv',encoding='gbk')
df.info()

#2. 特征预处理
#2.1 增加labels列，充当：标签列 好评->1 差评->0
df['labels'] = np.where(df['评价'] == '好评',1,0)
#2.2 抽取 labels列，作为：标签
y = df['labels']
#2.3 演示jieba分词
#print(jieba.lcut('小明，一把把把把住了'))
#2.4 对用户的评论信息进行切词
comment_list = [''.join(jieba.lcut(line)) for line in df['内容']]
#2.5 加载 停用词列表，即：里边记录的词，不需要参与模型训练，预测，要被删除的词，例如：的，啊，哈等...
with open('E:\\To_learn_Ai\\ML_Learning\\Naive_Bayes_Learning\\data\\stopwords.txt',
          'r',
...
#2.6 创建向量化对象，从 评论切词列表(comment_list)中 删除停用词，并统计词频
